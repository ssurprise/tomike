...
public class CustomCommentLayoutView extends LinearLayout {
    ...
    public CustomCommentLayoutView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mTitleHeightPx = DpPxSpToolKt.dip2px(context, 56);
        mSecondChildOffsetY = DpPxSpToolKt.dip2px(context, 210);
        mChildStartChangeThreshold = DpPxSpToolKt.dip2px(context, 30);
            ...
            for (int i = 0, j = getChildCount(); i < j; i++) {
                ...
                if (i == getChildCount() - 1) {
                    if (view.getVisibility() != GONE) {
                        minLocalHeight += view.getMeasuredHeight() + DpPxSpToolKt.dip2px(mContext, 30);
                    }
                }
            }
        ...
        if (child != null && rect.top <= mTotalScrollRange) {
            if (rect.top <= mChildStartChangeThreshold) {
                ...
                if (child instanceof LuCommentScoreWidget) {
                    ...
                    // 子view 滑动的区间
                    int moveRange = mTitleHeightPx / 2 + DpPxSpToolKt.dip2px(mContext, 25) / 2;
    ...
    private void refreshSecondChildView(Rect rect) {
        ...
        if (child != null && rect.top <= mTotalScrollRange) {
            if (rect.top <= mChildStartChangeThreshold) {
                ...
                int moveRange = mSecondChildOffsetY - mTitleHeightPx - DpPxSpToolKt.dip2px(mContext, 30);

                // 子View 的滑动偏移量
                int i = (rect.top - mChildStartChangeThreshold) * moveRange / (mTotalScrollRange - mChildStartChangeThreshold);

                // 子view 滑动，因为是向上滑动的，所以应当是减去滑动偏移，同时要减去之前未滑动的距离
                child.setTranslationY(rect.top - DpPxSpToolKt.dip2px(mContext, 30) - i);
                // 子View 形变
