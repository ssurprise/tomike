...
public class PhotoAlbumHelper{
	...
	private void getThumbnail() {
		...
		if (cursor1.moveToFirst()) {
			int image_id;
			String image_path;
			int image_idColumn = cursor1.getColumnIndex(Thumbnails.IMAGE_ID);
...
/*	void getAlbum() {
		...
		Cursor cursor1 = cr.query(Albums.EXTERNAL_CONTENT_URI, projection, null, null, Albums.ALBUM_ID + " desc");
	...
	public String getOriginalImagePath(String image_id) {
		...
		Cursor cursor = cr.query(Media.EXTERNAL_CONTENT_URI, projection, Media._ID + "=" + image_id, null, Media.DATE_MODIFIED + " desc");
		if (cursor != null) {
			cursor.moveToFirst();
